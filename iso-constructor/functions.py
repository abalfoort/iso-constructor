...
try:
    ...
    import fnmatch
    import glob
    from os.path import join, exists, abspath, splitext, expanduser, isdir, islink, lexists
...
def open_as_user(path_or_url):
    user = os.getlogin()
    command = "sudo -H -u {0} bash -c 'xdg-open \"{1}\"'".format(user, path_or_url)
...
def getLinuxHeadersAndImage(getLatest=False, includeLatestRegExp='', excludeLatestRegExp=''):
    ...
    if getLatest:
        ...
        for item in lst:
            lhMatch = re.search(r'linux-headers-\d+\.[a-zA-Z0-9-\.]*', item)
            if lhMatch:
                lh = lhMatch.group(0)
                addLh = True
...
def getVideoCards(pciId=None):
    ...
    for line in hwVideo:
        videoMatch = re.search(r':\s(.*)\[(\w*):(\w*)\]', line)
...
def get_lsb_release_info(lsb_release_path=None):
    lsb_dict = {}
    if not exists(lsb_release_path):
        lsb_release_path = '/etc/*release'
    try:
        lsb_dict['name'] = ec.run("egrep '^DISTRIB_DESCRIPTION|^PRETTY_NAME' /etc/*release | head -n 1 | cut -d'=' -f 2  | tr -d '\"'")[0]
        lsb_dict['id'] = ec.run("egrep '^DISTRIB_ID|^ID' /etc/*release | head -n 1 | cut -d'=' -f 2  | tr -d '\"'")[0]
        lsb_dict['codename'] = ec.run("egrep '^DISTRIB_CODENAME|^VERSION_CODENAME' /etc/*release | head -n 1 | cut -d'=' -f 2  | tr -d '\"'")[0]
        lsb_dict['version'] = ec.run("egrep '^DISTRIB_RELEASE|^VERSION_ID|^VERSION' /etc/*release | head -n 1 | cut -d'=' -f 2  | tr -d '\"'")[0]
...
def getPackageDependencies(packageName, reverseDepends=False):
    ...
    try:
        if reverseDepends:
            cmd = r'apt-cache rdepends %s | grep "^ "' % packageName
            depList = ec.run(cmd, False)
            if depList:
                for line in depList:
                    if line[0:2] != 'E:':
                        matchObj = re.search(r'([a-z0-9\-]+)', line)
                        if matchObj:
                            if matchObj.group(1) != '':
...
def getWirelessInterface():
    ...
    for line in reversed(wiList):
        if not rtsFound:
            reObj = re.search(r'\bRTS\b', line)
            if reObj:
                rtsFound = True
        else:
            reObj = re.search(r'^[a-z0-9]+', line)
            if reObj:
                wi = reObj.group(0)
                break
    return wi
...
def getDivertedFiles(mustContain=None):
    divertedFiles = []
    cmd = 'LANG=C dpkg-divert --list'
    if mustContain:
        cmd = 'LANG=C dpkg-divert --list | grep %s | cut -d' ' -f3' % mustContain
...
def getNetworkInterface():
    ...
    for line in reversed(ifList):
        if not found:
            reObj = re.search(r'inet.*bcast.*', line, re.I)
            if reObj:
                found = True
        else:
            reObj = re.search(r'^[a-z0-9]+', line, re.I)
            if reObj:
